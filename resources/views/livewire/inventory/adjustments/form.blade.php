        ...
        <div class="grid grid-cols-12 items-center gap-6">
            ...
            <div class="col-span-9 flex items-center justify-between">
                <div class="flex flex-wrap items-center gap-2">
                    ...
                    @if($adjustmentId && $status !== 'cancelled')
                    ...
                    <div class="grid gap-6 sm:grid-cols-2">
                        ...
                        <div>
                            ...
                            <div class="relative" x-data="{ open: false }">
                                ...
                                >
                                    @php $selectedWh = $warehouses->firstWhere('id', $warehouse_id); @endphp
                                    @if($selectedWh)
                                ...
                                <div x-show="open" @click.outside="open = false" x-transition class="absolute left-0 top-full z-50 mt-1 w-full rounded-lg border border-zinc-200 bg-white py-1 shadow-lg dark:border-zinc-700 dark:bg-zinc-900">
                                    @foreach($warehouses as $warehouse)
                                        <button type="button" wire:click="$set('warehouse_id', {{ $warehouse->id }})" @click="open = false" class="block w-full px-4 py-2 text-left text-sm text-zinc-700 hover:bg-zinc-50 dark:text-zinc-300 dark:hover:bg-zinc-800">{{ $warehouse->name }}</button>
                                    @endforeach
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="space-y-3">
                            ...
                            <div class="flex items-center gap-4">
                                <label class="w-28 flex-shrink-0 text-sm font-light text-zinc-600 dark:text-zinc-400">Date</label>
                                <div class="relative flex-1">
                                    <input type="date" wire:model="adjustment_date" class="w-full rounded-lg border border-transparent bg-transparent px-3 py-1.5 text-sm text-zinc-900 transition-colors hover:border-zinc-200 focus:border-zinc-400 focus:outline-none dark:text-zinc-100 dark:hover:border-zinc-700" />
                                </div>
                            </div>
                        ...
                        <div class="sm:col-span-2">
                            <label class="mb-2 block text-sm font-light text-zinc-600 dark:text-zinc-400">Reason</label>
                            <input type="text" wire:model="reason" placeholder="Reason for adjustment..." class="w-full rounded-lg border border-zinc-200 bg-white px-3 py-2 text-sm text-zinc-900 placeholder-zinc-400 focus:border-zinc-400 focus:outline-none dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-100" />
                        </div>
                    </div>
                </div>

                {{-- Tab Headers: Adjustment Items & Other Info --}}
                        ...
                        <table class="w-full">
                            <thead>
                                <tr class="border-b border-zinc-100 bg-zinc-50/50 dark:border-zinc-800 dark:bg-zinc-900/50">
                                    ...
                                    <th class="w-28 px-3 py-2.5 text-right text-xs font-medium uppercase tracking-wider text-zinc-500 dark:text-zinc-400">System Qty</th>
                                    <th class="w-28 px-3 py-2.5 text-right text-xs font-medium uppercase tracking-wider text-zinc-500 dark:text-zinc-400">Counted Qty</th>
                                    <th class="w-24 px-3 py-2.5 text-right text-xs font-medium uppercase tracking-wider text-zinc-500 dark:text-zinc-400">Difference</th>
                            ...
                            <tbody class="divide-y divide-zinc-50 dark:divide-zinc-800/50">
                                @forelse($items as $index => $item)
                                    <tr class="group hover:bg-zinc-50/50 dark:hover:bg-zinc-800/30" wire:key="item-{{ $index }}">
                                        ...
                                        {{-- System Quantity --}}
                                        <td class="px-3 py-2">
                                            <input type="text" wire:model="items.{{ $index }}.system_quantity" readonly class="w-full bg-transparent text-right text-sm text-zinc-500 focus:outline-none dark:text-zinc-400" />
                                        </td>

                                        {{-- Counted Quantity --}}
                                        <td class="px-3 py-2">
                                            <input type="text" wire:model.live="items.{{ $index }}.counted_quantity" class="w-full bg-transparent text-right text-sm text-zinc-900 focus:outline-none dark:text-zinc-100" />
                                        </td>
                ...
                <div x-show="activeTab === 'other'" x-cloak x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100">
                    <div class="p-6">
                        <div class="grid gap-8 lg:grid-cols-2">
                            ...
                            <div class="space-y-4">
                                <h3 class="text-sm font-medium uppercase tracking-wider text-zinc-500 dark:text-zinc-400">Adjustment Details</h3>
                                <div class="space-y-4">
                                    <div>
                            ...
                            {{-- Approval Section --}}
                            <div class="space-y-4">
                                <h3 class="text-sm font-medium uppercase tracking-wider text-zinc-500 dark:text-zinc-400">Approval</h3>
                                <div class="space-y-4">
                                    <div>
                                        <label class="mb-1.5 block text-sm text-zinc-600 dark:text-zinc-400">Approved By</label>
                                        <input type="text" placeholder="Approver name..." class="w-full rounded-lg border border-zinc-200 bg-white px-3 py-2 text-sm text-zinc-900 placeholder-zinc-400 focus:border-zinc-400 focus:outline-none dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-100" />
                                    </div>
                                    <div>
                                        <label class="mb-1.5 block text-sm text-zinc-600 dark:text-zinc-400">Approval Date</label>
                                        <input type="date" class="w-full rounded-lg border border-zinc-200 bg-white px-3 py-2 text-sm text-zinc-900 focus:border-zinc-400 focus:outline-none dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-100" />
                                    </div>
                                </div>
                            </div>
                        </div>
        ...
        <div class="lg:col-span-3">
            ...
            <div x-show="showSendMessage" x-collapse class="mb-4">
                <div class="flex gap-3">
                    ...
                    <div class="flex-1">
                        <textarea rows="3" placeholder="Send a message..." class="w-full resize-none rounded-lg border border-zinc-200 bg-white px-3 py-2 text-sm placeholder-zinc-400 focus:border-blue-400 focus:outline-none dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-100"></textarea>
                        <div class="mt-2 flex justify-end">
                            <button type="button" class="rounded-lg bg-blue-600 px-3 py-1.5 text-xs font-medium text-white hover:bg-blue-700">Send</button>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div x-show="showLogNote" x-collapse class="mb-4">
                <div class="flex gap-3">
                    ...
                    <div class="flex-1">
                        <textarea rows="3" placeholder="Log an internal note..." class="w-full resize-none rounded-lg border border-zinc-200 bg-white px-3 py-2 text-sm placeholder-zinc-400 focus:border-amber-400 focus:outline-none dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-100"></textarea>
                        <div class="mt-2 flex justify-end">
                            <button type="button" class="rounded-lg bg-amber-600 px-3 py-1.5 text-xs font-medium text-white hover:bg-amber-700">Log Note</button>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div x-show="showScheduleActivity" x-collapse class="mb-4">
                <div class="flex gap-3">
                    ...
                    <div class="flex-1 space-y-3">
                        <select class="w-full rounded-lg border border-zinc-200 bg-white px-3 py-2 text-sm focus:border-violet-400 focus:outline-none dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-100">
                            <option value="">Select activity type...</option>
                            <option value="call">Call</option>
                            <option value="meeting">Meeting</option>
                            <option value="todo">To-Do</option>
                        </select>
                        <input type="date" class="w-full rounded-lg border border-zinc-200 bg-white px-3 py-2 text-sm focus:border-violet-400 focus:outline-none dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-100" />
                        <div class="flex justify-end">
                            <button type="button" class="rounded-lg bg-violet-600 px-3 py-1.5 text-xs font-medium text-white hover:bg-violet-700">Schedule</button>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="space-y-4">
                    <div class="flex gap-3">
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <p class="text-sm text-zinc-600 dark:text-zinc-400">Adjustment created</p>
                        </div>
                    </div>
                </div>
            @else
                <div class="py-8 text-center">
                    ...
                    <p class="text-xs text-zinc-400 dark:text-zinc-500">Activity will appear here once the adjustment is saved</p>
                </div>
            @endif
        </div>
        </div>
