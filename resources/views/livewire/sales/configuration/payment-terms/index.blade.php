        ...
        <x-slot:search>
                @if(count($selected) > 0)
                    <div class="flex items-center gap-2">
                        ...
                        <button wire:click="deleteSelected" wire:confirm="Are you sure you want to delete {{ count($selected) }} payment term(s)?" class="inline-flex items-center gap-1.5 rounded-lg border border-red-200 bg-white px-3 py-1.5 text-sm font-medium text-red-600 hover:bg-red-50 dark:border-red-800 dark:bg-zinc-800 dark:text-red-400 dark:hover:bg-red-900/20">
                            <flux:icon name="trash" class="size-4" />
                            Delete
                        </button>
                    </div>
                @else
                    <x-ui.searchbox-dropdown placeholder="Search payment terms..." widthClass="w-[520px]" width="520px" :activeFilterCount="$this->getActiveFilterCount()" clearAction="clearFilters">
        ...
        <table class="min-w-full divide-y divide-zinc-200 dark:divide-zinc-800">
            <thead class="border-b border-zinc-200 bg-zinc-50 dark:border-zinc-800 dark:bg-zinc-950">
                <tr>
                    ...
                    <th scope="col" class="px-4 py-3 text-center text-xs font-bold uppercase tracking-wider text-zinc-500 dark:text-zinc-400">Days</th>
            ...
            <tbody class="divide-y divide-zinc-200 dark:divide-zinc-800">
                @forelse ($paymentTerms as $term)
                    ...
                    <tr onclick="window.location.href='{{ route('sales.configuration.payment-terms.edit', $term->id) }}'" class="group cursor-pointer transition-all duration-150 {{ $isSelected ? 'bg-zinc-900/[0.03] dark:bg-zinc-100/[0.03]' : 'hover:bg-zinc-50 dark:hover:bg-zinc-800/50' }}">
                        ...
                        <td class="px-4 py-4">
                            <span class="inline-flex items-center rounded bg-zinc-100 px-2 py-0.5 text-xs font-medium text-zinc-600 dark:bg-zinc-800 dark:text-zinc-400">{{ $term->code }}</span>
                        </td>
                        <td class="px-4 py-4 text-center">
                            <span class="text-sm text-zinc-600 dark:text-zinc-400">{{ $term->formatted_days }}</span>
                        </td>
                        <td class="px-4 py-4">
                            @if($term->is_active)
                    ...
                    <tr>
                        <td colspan="6" class="px-6 py-12 text-center">
                            <div class="flex flex-col items-center gap-3">
                                ...
                                <div>
                                    <p class="text-sm font-normal text-zinc-900 dark:text-zinc-100">No payment terms found</p>
                                    <p class="text-xs font-light text-zinc-500 dark:text-zinc-400">Create your first payment term</p>
                                </div>
                            </div>
                        </td>
                    </tr>
