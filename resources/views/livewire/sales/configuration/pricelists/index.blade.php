    ...
    <x-slot:header>
        <div class="flex items-center justify-between gap-4">
            ...
            <div class="flex items-center gap-3">
                <a href="{{ route('sales.configuration.pricelists.create') }}" wire:navigate class="inline-flex items-center justify-center rounded-lg bg-zinc-900 px-3 py-1.5 text-sm font-medium text-white shadow-sm hover:bg-zinc-800 dark:bg-zinc-100 dark:text-zinc-900 dark:hover:bg-zinc-200">
                    New
                </a>
            ...
            <div class="flex flex-1 items-center justify-center">
                ...
                @else
                    <div class="relative flex h-9 w-[400px] items-center overflow-hidden rounded-lg border border-zinc-200 bg-white dark:border-zinc-700 dark:bg-zinc-800">
                        ...
                        <input type="text" wire:model.live.debounce.300ms="search" placeholder="Search pricelists..." class="h-full w-full border-0 bg-transparent pl-9 pr-4 text-sm outline-none focus:ring-0" />
                    </div>
                @endif
            </div>
            ...
            <div class="flex items-center gap-3">
                @if($pricelists->hasPages())
                    <div class="flex items-center gap-1">
                        <button wire:click="previousPage" @disabled($pricelists->onFirstPage()) class="flex h-8 w-8 items-center justify-center rounded-lg border border-zinc-200 text-zinc-600 disabled:opacity-50 dark:border-zinc-700 dark:text-zinc-400">
                            <flux:icon name="chevron-left" class="size-4" />
                        </button>
                        <button wire:click="nextPage" @disabled(!$pricelists->hasMorePages()) class="flex h-8 w-8 items-center justify-center rounded-lg border border-zinc-200 text-zinc-600 disabled:opacity-50 dark:border-zinc-700 dark:text-zinc-400">
        ...
        <table class="min-w-full divide-y divide-zinc-200 dark:divide-zinc-800">
            ...
            <tbody class="divide-y divide-zinc-200 dark:divide-zinc-800">
                @forelse ($pricelists as $pricelist)
                    <tr onclick="window.location.href='{{ route('sales.configuration.pricelists.edit', $pricelist->id) }}'" class="cursor-pointer transition-colors hover:bg-zinc-50 dark:hover:bg-zinc-800/50">
