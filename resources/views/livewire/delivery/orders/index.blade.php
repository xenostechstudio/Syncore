            ...
            <table class="w-full">
                <thead>
                    <tr class="border-b border-zinc-100 bg-zinc-50 dark:border-zinc-800 dark:bg-zinc-900">
                        <th class="px-5 py-3 text-left text-xs font-normal text-zinc-500 dark:text-zinc-400">Delivery</th>
                        <th class="px-5 py-3 text-left text-xs font-normal text-zinc-500 dark:text-zinc-400">Sales Order</th>
                        <th class="px-5 py-3 text-left text-xs font-normal text-zinc-500 dark:text-zinc-400">Recipient</th>
                        <th class="px-5 py-3 text-left text-xs font-normal text-zinc-500 dark:text-zinc-400">Courier</th>
                        <th class="px-5 py-3 text-left text-xs font-normal text-zinc-500 dark:text-zinc-400">Delivery Date</th>
                        ...
                        <tr class="transition-colors hover:bg-zinc-50 dark:hover:bg-zinc-800/50">
                                ...
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        @if($delivery->tracking_number)
                                            <p class="text-xs font-light text-zinc-500 dark:text-zinc-400">{{ $delivery->tracking_number }}</p>
                                        @endif
                                    </div>
                                </div>
                            </td>
                            <td class="px-5 py-4">
                                <a href="#" class="text-sm font-light text-blue-600 hover:underline dark:text-blue-400">{{ $delivery->salesOrder->order_number }}</a>
                            </td>
                            <td class="px-5 py-4">
                                <p class="text-sm font-normal text-zinc-900 dark:text-zinc-100">{{ $delivery->recipient_name ?? $delivery->salesOrder->customer->name }}</p>
                                <p class="text-xs font-light text-zinc-500 dark:text-zinc-400">{{ $delivery->recipient_phone }}</p>
                            </td>
                            <td class="px-5 py-4">
                                <span class="text-sm font-light text-zinc-600 dark:text-zinc-300">{{ $delivery->courier }}</span>
                            </td>
                            <td class="px-5 py-4">
                                <p class="text-sm font-light text-zinc-600 dark:text-zinc-300">{{ $delivery->delivery_date->format('M d, Y') }}</p>
                                @if($delivery->actual_delivery_date)
                                    <p class="text-xs font-light text-emerald-600 dark:text-emerald-400">Delivered {{ $delivery->actual_delivery_date->format('M d') }}</p>
                                @endif
                            </td>
                            <td class="px-5 py-4">
                                @php
                            ...
                            <td class="px-5 py-4 text-right">
                                <div class="flex items-center justify-end gap-1">
                                    ...
                                    <button class="rounded p-1.5 text-zinc-400 transition-colors hover:bg-zinc-100 hover:text-zinc-600 dark:hover:bg-zinc-800 dark:hover:text-zinc-300" title="Track">
                                        <svg class="size-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
                        ...
                        <tr>
                            <td colspan="7" class="px-5 py-12 text-center">
                                <div class="flex flex-col items-center gap-3">
                                    ...
                                    <div>
                                        <p class="text-sm font-normal text-zinc-900 dark:text-zinc-100">No deliveries found</p>
        ...
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
            @forelse ($deliveries as $delivery)
                <div class="rounded-lg border border-zinc-200 bg-white p-5 transition-all hover:border-zinc-300 hover:shadow-sm dark:border-zinc-800 dark:bg-zinc-900 dark:hover:border-zinc-700">
                    <div class="mb-4 flex items-start justify-between">
                        <div>
                            <p class="text-sm font-normal text-zinc-900 dark:text-zinc-100">{{ $delivery->delivery_number }}</p>
                            <p class="text-xs font-light text-zinc-500 dark:text-zinc-400">{{ $delivery->delivery_date->format('M d, Y') }}</p>
                        </div>
                        @php
                            $statusConfig = match($delivery->status) {
                                'pending' => ['bg' => 'bg-zinc-100 dark:bg-zinc-800', 'text' => 'text-zinc-600 dark:text-zinc-400', 'label' => 'Pending'],
                    ...
                    <div class="mb-4 space-y-2">
                        <div class="flex items-center gap-2 text-sm">
                            ...
                            <span class="font-light text-zinc-600 dark:text-zinc-300">{{ $delivery->recipient_name ?? $delivery->salesOrder->customer->name }}</span>
                        </div>
                        <div class="flex items-center gap-2 text-sm">
                            <svg class="size-4 text-zinc-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M8.25 18.75a1.5 1.5 0 01-3 0m3 0a1.5 1.5 0 00-3 0m3 0h6m-9 0H3.375a1.125 1.125 0 01-1.125-1.125V14.25m17.25 4.5a1.5 1.5 0 01-3 0m3 0a1.5 1.5 0 00-3 0m3 0h1.125c.621 0 1.129-.504 1.09-1.124a17.902 17.902 0 00-3.213-9.193 2.056 2.056 0 00-1.58-.86H14.25M16.5 18.75h-2.25m0-11.177v-.958c0-.568-.422-1.048-.987-1.106a48.554 48.554 0 00-10.026 0 1.106 1.106 0 00-.987 1.106v7.635m12-6.677v6.677m0 4.5v-4.5m0 0h-12" />
                            </svg>
                            <span class="font-light text-zinc-600 dark:text-zinc-300">{{ $delivery->courier }}</span>
                        </div>
                        @if($delivery->tracking_number)
                            <div class="flex items-center gap-2 text-sm">
                                <svg class="size-4 text-zinc-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
                                ...
                                <code class="font-light text-zinc-500 dark:text-zinc-400">{{ $delivery->tracking_number }}</code>
                            </div>
                        @endif
                    </div>

                    <div class="flex items-center justify-between border-t border-zinc-100 pt-4 dark:border-zinc-800">
                        <a href="#" class="text-xs font-light text-blue-600 hover:underline dark:text-blue-400">{{ $delivery->salesOrder->order_number }}</a>
            ...
            @empty
                <div class="col-span-full py-12 text-center">
                    <p class="text-sm font-light text-zinc-500 dark:text-zinc-400">No deliveries found</p>
                </div>
            @endforelse
        </div>
    @endif
    ...
    @if($deliveries->hasPages())
        <div class="flex items-center justify-between border-t border-zinc-200 pt-4 dark:border-zinc-800">
            <p class="text-sm font-light text-zinc-500 dark:text-zinc-400">
                Showing {{ $deliveries->firstItem() }} to {{ $deliveries->lastItem() }} of {{ $deliveries->total() }} results
            </p>
            {{ $deliveries->links('livewire.pagination') }}
        </div>
    @endif
</div>
