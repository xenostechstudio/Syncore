<div>
    <x-ui.flash />    <x-ui.index-header
        ...
        :createRoute="route('purchase.rfq.create')"
        :paginator="$rfqs"
        :view="$view"
        :views="['list']"
    >
            ...
            <div class="-mx-4 -mt-6 -mb-6 overflow-x-auto bg-white sm:-mx-6 lg:-mx-8 dark:bg-zinc-900">
                <table class="min-w-full divide-y divide-zinc-200 dark:divide-zinc-800">
                    ...
                    <tbody class="divide-y divide-zinc-200 dark:divide-zinc-800">
                        @forelse ($rfqs as $rfq)
                            @php $isSelected = in_array($rfq->id, $selected); @endphp
                            <tr 
                            ...
                            >
                                ...
                                @if($visibleColumns['status'])
                                    <td class="px-4 py-4">
                                        <x-ui.status-badge :status="\App\Enums\PurchaseOrderState::tryFrom($rfq->status)" class="px-2.5 py-0.5" />
                                    </td>
                                @endif
