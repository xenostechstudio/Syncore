    ...
    <div class="sticky top-14 z-40 -mx-4 -mt-6 mb-6 flex min-h-[60px] items-center border-b border-zinc-200 bg-white px-4 py-2 sm:-mx-6 lg:-mx-8 lg:px-6 dark:border-zinc-800 dark:bg-zinc-950">
        <div class="flex w-full items-center justify-between gap-4">
            ...
            <div class="flex items-center gap-3">
                <a href="{{ route('hr.positions.create') }}" wire:navigate class="inline-flex items-center justify-center rounded-lg bg-zinc-900 px-3 py-1.5 text-sm font-medium text-white shadow-sm hover:bg-zinc-800 dark:bg-zinc-100 dark:text-zinc-900 dark:hover:bg-zinc-200">
                    New
                </a>
                <span class="text-md font-light text-zinc-600 dark:text-zinc-400">Positions</span>
                        ...
                        @if($departmentId)
                            ...
                            <div class="flex items-center">
                                <span class="inline-flex h-6 items-center gap-1.5 rounded-md bg-zinc-900 px-2 text-[10px] font-semibold text-white shadow-sm dark:bg-zinc-100 dark:text-zinc-900">
                                    ...
                                    <button type="button" onclick="event.stopPropagation()" wire:click="$set('departmentId', '')" class="-mr-0.5 inline-flex h-4 w-4 items-center justify-center rounded-md text-zinc-400 hover:bg-zinc-200 hover:text-zinc-600 dark:hover:bg-zinc-800 dark:hover:text-zinc-200">
            ...
            <div class="flex items-center gap-3">
                <div class="flex items-center gap-2">
                    <span class="text-sm text-zinc-500 dark:text-zinc-400">
                        {{ $positions->firstItem() ?? 0 }}-{{ $positions->lastItem() ?? 0 }}/{{ $positions->total() }}
                    </span>
                    <div class="flex items-center gap-0.5">
                        <button type="button" wire:click="goToPreviousPage" @disabled($positions->onFirstPage()) class="flex h-7 w-7 items-center justify-center rounded text-zinc-400 transition-colors hover:bg-zinc-100 hover:text-zinc-600 disabled:cursor-not-allowed disabled:opacity-40 dark:hover:bg-zinc-800 dark:hover:text-zinc-300">
                            <flux:icon name="chevron-left" class="size-4" />
                        </button>
                        <button type="button" wire:click="goToNextPage" @disabled(!$positions->hasMorePages()) class="flex h-7 w-7 items-center justify-center rounded text-zinc-400 transition-colors hover:bg-zinc-100 hover:text-zinc-600 disabled:cursor-not-allowed disabled:opacity-40 dark:hover:bg-zinc-800 dark:hover:text-zinc-300">
        ...
        @if($view === 'grid')
            <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                @forelse($positions as $position)
                    <a href="{{ route('hr.positions.edit', $position->id) }}" wire:navigate class="group rounded-xl border border-zinc-200 bg-white p-4 transition-all hover:border-zinc-300 hover:shadow-md dark:border-zinc-800 dark:bg-zinc-900 dark:hover:border-zinc-700">
                        <div class="flex items-start justify-between">
                            <div>
                                <p class="text-sm font-medium text-zinc-900 dark:text-zinc-100">{{ $position->name }}</p>
                                <p class="text-xs text-zinc-500 dark:text-zinc-400">{{ $position->department?->name ?? '-' }}</p>
                            </div>
                        ...
                        <div class="mt-3 flex items-center justify-between text-xs text-zinc-500 dark:text-zinc-400">
                            <span>{{ $position->employees->count() }} employees</span>
                            @if($position->min_salary || $position->max_salary)
                                <span>Rp {{ number_format($position->min_salary ?? 0, 0, ',', '.') }}+</span>
                            @endif
                        </div>
                    </a>
                @empty
                    <div class="col-span-full py-12 text-center">
                        <flux:icon name="briefcase" class="mx-auto size-12 text-zinc-300 dark:text-zinc-600" />
                        <p class="mt-2 text-sm text-zinc-500">No positions found</p>
                    </div>
                @endforelse
            </div>
        @else
                ...
                <table class="min-w-full divide-y divide-zinc-200 dark:divide-zinc-800">
                    <thead class="border-b border-zinc-200 bg-zinc-50 dark:border-zinc-800 dark:bg-zinc-950">
                        <tr>
                            ...
                            <th scope="col" class="py-3 pl-2 pr-4 text-left text-xs font-bold uppercase tracking-wider text-zinc-500 dark:text-zinc-400">Position</th>
                            ...
                            <th scope="col" class="px-4 py-3 text-left text-xs font-bold uppercase tracking-wider text-zinc-500 dark:text-zinc-400">Salary Range</th>
                    ...
                    <tbody class="divide-y divide-zinc-200 dark:divide-zinc-800">
                        @forelse($positions as $position)
                            ...
                            <tr wire:key="pos-{{ $position->id }}" onclick="window.Livewire.navigate('{{ route('hr.positions.edit', $position->id) }}')" class="group cursor-pointer transition-all duration-150 {{ $isSelected ? 'bg-zinc-900/[0.03] dark:bg-zinc-100/[0.03]' : 'hover:bg-zinc-50 dark:hover:bg-zinc-800/50' }}">
                                ...
                                <td class="px-4 py-3 text-sm text-zinc-600 dark:text-zinc-400">
                                    @if($position->min_salary || $position->max_salary)
                                        Rp {{ number_format($position->min_salary ?? 0, 0, ',', '.') }} - {{ number_format($position->max_salary ?? 0, 0, ',', '.') }}
                                    @else
                                        -
                                    @endif
                                </td>
                                <td class="px-4 py-3 text-sm text-zinc-600 dark:text-zinc-400">{{ $position->employees->count() }}</td>
                                <td class="px-4 py-3">
                                    @if($position->is_active)
                                        <span class="inline-flex rounded-full bg-emerald-100 px-2 py-0.5 text-xs font-medium text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400">Active</span>
                                    @else
                        ...
                        @empty
                            <tr>
                                <td colspan="6" class="px-4 py-8 text-center text-sm text-zinc-400">No positions found</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
