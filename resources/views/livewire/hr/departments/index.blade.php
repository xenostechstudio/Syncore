            ...
            <div class="-mx-4 -mt-6 -mb-6 overflow-x-auto bg-white sm:-mx-6 lg:-mx-8 dark:bg-zinc-900">
                <table class="min-w-full divide-y divide-zinc-200 dark:divide-zinc-800">
                    ...
                    <tbody class="divide-y divide-zinc-200 dark:divide-zinc-800">
                        @forelse($departments as $dept)
                            @php $isSelected = in_array($dept->id, $selected); @endphp
                            <tr wire:key="dept-{{ $dept->id }}" onclick="window.Livewire.navigate('{{ route('hr.departments.edit', $dept->id) }}')" class="group cursor-pointer transition-all duration-150 {{ $isSelected ? 'bg-zinc-900/[0.03] dark:bg-zinc-100/[0.03]' : 'hover:bg-zinc-50 dark:hover:bg-zinc-800/50' }}">
                                <td class="relative py-3 pl-4 pr-2 sm:pl-6 lg:pl-8" onclick="event.stopPropagation()">
                                    ...
                                    <input type="checkbox" wire:model.live="selected" value="{{ $dept->id }}" class="rounded border-zinc-300 bg-white text-zinc-900 focus:ring-zinc-900 dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-100 dark:focus:ring-zinc-600 {{ $isSelected ? 'ring-1 ring-zinc-900/20 dark:ring-zinc-100/20' : '' }}">
                                </td>
