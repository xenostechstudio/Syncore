    ...
    <div class="container">
        ...
        <table style="margin-bottom: 30px;">
            <tr>
                ...
                <td style="border: none; padding: 0; width: 50%; vertical-align: top; text-align: right;">
                    ...
                    <div class="info-value">
                        Order Date: {{ $order->order_date?->format('M d, Y') }}<br>
                        @if($order->delivery_date)
                        Delivery Date: {{ $order->delivery_date->format('M d, Y') }}<br>
                        @endif
        ...
        <div class="totals">
            <div class="totals-row">
                ...
                <span class="totals-value">Rp{{ number_format($order->subtotal ?? 0, 0, ',', '.') }}</span>
            </div>
            @if($order->tax_amount > 0)
            <div class="totals-row">
                <span class="totals-label">Tax</span>
                <span class="totals-value">Rp{{ number_format($order->tax_amount, 0, ',', '.') }}</span>
            </div>
            @endif
            @if($order->discount_amount > 0)
            <div class="totals-row">
                <span class="totals-label">Discount</span>
                <span class="totals-value">-Rp{{ number_format($order->discount_amount, 0, ',', '.') }}</span>
            </div>
            @endif
