...
@php
    ...
    // Module Configuration items (sidebar)
    $moduleConfigItems = [
        [
            'label' => 'Sales Order',
    ...
    // General Settings items (sidebar) — Users & Roles live in top-nav Account Management dropdown
    ...
    $isAccountManagementActive = request()->routeIs('settings.users*') || request()->routeIs('settings.roles*');
@endphp
    ...
    <body class="flex h-screen flex-col overflow-hidden bg-zinc-50 dark:bg-zinc-900">
        ...
        <header class="flex-shrink-0 bg-white dark:bg-zinc-950">
            <div class="flex h-14 items-center px-4 lg:px-6">
                ...
                {{-- Middle: Top Navigation (Overview + Account Management hover dropdown) --}}
                <nav class="relative hidden h-full items-center md:flex">
                    <div class="relative flex h-full items-center gap-1">
                        ...
                        >
                            ...
                            <span>{{ __('nav.overview') }}</span>
                        </a>

                        {{-- Account Management (hover dropdown) --}}
                        <div class="group relative flex h-full items-center">
                            <a
                                href="{{ route('settings.users.index') }}"
                                wire:navigate
                                class="inline-flex h-full items-center gap-1.5 border-b-2 px-3 text-sm font-normal transition-colors whitespace-nowrap {{ $isAccountManagementActive ? 'border-zinc-900 text-zinc-900 dark:border-zinc-100 dark:text-zinc-100' : 'border-transparent text-zinc-500 hover:text-zinc-700 dark:text-zinc-400 dark:hover:text-zinc-300' }}"
                            >
                                <flux:icon name="users" class="size-4" />
                                <span>{{ __('nav.account_management') }}</span>
                            </a>
                            ...
                            <div class="invisible absolute left-0 top-full z-50 min-w-[180px] rounded-lg border border-zinc-200 bg-white py-1 opacity-0 shadow-lg transition-all group-hover:visible group-hover:opacity-100 dark:border-zinc-700 dark:bg-zinc-900">
                                @php
                                    $usersActive = request()->routeIs('settings.users*');
                                    $rolesActive = request()->routeIs('settings.roles*');
                                @endphp
                                <a
                                    href="{{ route('settings.users.index') }}"
                                    wire:navigate
                                    class="block px-4 py-2 text-sm transition-colors {{ $usersActive ? 'bg-zinc-100 text-zinc-900 dark:bg-zinc-800 dark:text-zinc-100' : 'text-zinc-600 hover:bg-zinc-50 hover:text-zinc-900 dark:text-zinc-400 dark:hover:bg-zinc-800 dark:hover:text-zinc-100' }}"
                                >
                                    {{ __('nav.users') }}
                                </a>
                                <a
                                    ...
                                    wire:navigate
                                    class="block px-4 py-2 text-sm transition-colors {{ $rolesActive ? 'bg-zinc-100 text-zinc-900 dark:bg-zinc-800 dark:text-zinc-100' : 'text-zinc-600 hover:bg-zinc-50 hover:text-zinc-900 dark:text-zinc-400 dark:hover:bg-zinc-800 dark:hover:text-zinc-100' }}"
                                >
                                    {{ __('nav.roles_permissions') }}
                                </a>
                            </div>
                        </div>
                    </div>
